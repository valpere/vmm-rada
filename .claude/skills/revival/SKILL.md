...
| **Vital organs** | `cmd/server/main.go` (DI wiring + registry), `internal/api/handler.go` (HTTP handlers + SSE) | Rada type registry, route registration, SSE event flow | `cat cmd/server/main.go`, `grep -n "mux.Handle\|func (h" internal/api/handler.go` |
