...
| Multiple deliberation strategies, operator-selectable per registration | 7 `Strategy` constants; see [`strategies.md`](./strategies.md) for the full per-strategy cost/quorum/config table |
...
|---|---|---|---|
| **RoleBased is unreachable** | `strategies.md`'s status table historically listed it as "shipped," same as the other six | Real, tested pipeline code (`internal/council/rolebased.go`) with **zero registration path** — no env var family, nothing in `cmd/server/main.go` ever constructs `CouncilType{Strategy: RoleBased}`. It cannot be invoked through the running server. | File a `/backlog` item: either register it (with a namespaced env var family, matching the pattern every other strategy uses) or formally retire it in favour of the "rebuilt on top of `Majority`/`MixtureOfAgents`" code-review plan mentioned in [`strategies.md`](./strategies.md#whats-not-here). |
